...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def __init__(self, min_size, max_size, image_mean, image_std):
        ...
        if not isinstance(min_size, (list, tuple)):
            min_size = (min_size,)    # 将min_size转化为tuple类型
        self.min_size = min_size      # 指定图像的最小边长范围
        self.max_size = max_size      # 指定图像的最大边长范围
        self.image_mean = image_mean  # 指定图像在标准化处理中的均值 [0.485, 0.456, 0.406]
        self.image_std = image_std    # 指定图像在标准化处理中的方差 [0.229, 0.224, 0.225]
    ...
    def resize(self, image, target):
        ...
        if target is None:  # 如果没有标注信息，就返回处理好的图像（image）和标注信息（target）
            return image, target
        # 否则继续处理标注框的缩放
        bbox = target["boxes"]
        # 根据图像的缩放比例来缩放bbox
