...
class AnchorsGenerator(nn.Module):
    ...
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0)):
        ...
        if not isinstance(sizes[0], (list, tuple)): # 判断sizes[0]是否为列表或者元组
            # TODO change this
            sizes = tuple((s,) for s in sizes)  # ((128,), (256,), (512,))
        if not isinstance(aspect_ratios[0], (list, tuple)): # 判断sizes[0]是否为列表或者元组
            aspect_ratios = (aspect_ratios,) * len(sizes)   # ((0.5, 1.0, 2.0),(0.5, 1.0, 2.0),...) 元组个数为len(sizes)
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        Arguments:
            ...
            device: cpu/gpu
        """
        scales = torch.as_tensor(scales, dtype=dtype, device=device)    # change type of scales into tensor
        aspect_ratios = torch.as_tensor(aspect_ratios, dtype=dtype, device=device)  # as above change type aspect_ratios into tensor
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            grid_height, grid_width = size  # 对应特征图的高度和宽度
            stride_height, stride_width = stride    # 每个cell对应原图的高度和宽度
...
class RPNHead(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        # 3x3 滑动窗口
        # Conv2d的参数有in_channels,    out_channels
        self.conv = nn.Conv2d(in_channels, in_channels, kernel_size=3, stride=1, padding=1)
        # 计算预测的目标分数（这里的目标只是指前景或者背景,即判断是不是检测目标）
