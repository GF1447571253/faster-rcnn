...
class FasterRCNNBase(nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        """
        if self.training and targets is None:   # 判断是否在训练时期并且targets不为空
        ...
        if self.training:
            ...
            for target in targets:         # 进一步判断传入的target的boxes参数是否符合规定
                boxes = target["boxes"]
                if isinstance(boxes, torch.Tensor):  # 判断boxes是否是torch.Tensor类型
        ...
        for img in images:  # images的维度为[channel,height,width]
            val = img.shape[-2:]
        ...
        losses = {}
        losses.update(detector_losses)  # fast rcnn损失
        losses.update(proposal_losses)  # rpn损失
...
class FasterRCNN(FasterRCNNBase):
    ...
    def __init__(self, backbone, num_classes=None,
        ...
        if not hasattr(backbone, "out_channels"):   # 判断对象backbone是否有out_channels属性
            raise ValueError(
        ...
        assert isinstance(rpn_anchor_generator, (AnchorsGenerator, type(None))) #判断rpn_anchor_generator是否是AnchorsGenerator或者type(None)类型
        assert isinstance(box_roi_pool, (MultiScaleRoIAlign, type(None)))   #判断box_roi_pool是否是MultiScaleRoIAlign或者type(None)类型
